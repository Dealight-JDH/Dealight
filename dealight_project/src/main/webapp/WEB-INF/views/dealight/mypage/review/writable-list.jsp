...
<body>
	...
	<c:forEach items="${rsvdList }" var="rsvd">
		...
		<button class="myBtn">
			<a href="register/rsvd?userId=<c:out value="${rsvd.userId }" />&rsvdId=<c:out value="${rsvd.id }" />&waitId=-1">
	...
	<c:forEach items="${waitList }" var="wait">
		...
		<button class="myBtn">
			<a href="register/wait?userId=<c:out value="${wait.userId }" />&waitId=<c:out value="${wait.id }" />&rsvdId=-1">
			리뷰 쓰기</a>
		</button>
		<br />
	</c:forEach>
