...
public interface RevwMapper {
	...
	// 리뷰 하나씩 읽어오기
	public RevwVO readRevw(int id);

	// 리뷰 등록에 불러올 예약 항목 (파라미터 2개 이상일 때 @Param 붙이기)
	public RsvdVO getWritableItemByRsvd(@Param("userId") String userId, @Param("id") int rsvdId);
	
	// 리뷰 등록에 불러올 웨이팅 항목
	public WaitVO getWritableItemByWait(@Param("userId") String userId, @Param("id") int waitId);
	...
	// 예약 항목 리뷰 상태 업데이트
	public int updateRsvdRevwStus(@Param("userId") String userId, @Param("id") int rsvdId);
	
	// 웨이팅 항목 리뷰 상태 업데이트
	public int updateWaitRevwStus(@Param("userId") String userId, @Param("id") int waitId);
	
	// 리뷰 답글 업데이트
	public int updateRevwReply(RevwVO revw);

	// 리뷰 삭제
	public int deleteRevw(int id);
